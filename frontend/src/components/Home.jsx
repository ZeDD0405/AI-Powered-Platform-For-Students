...
const Home = () => {
  ...
  const features = [
    ...
    },
    {
      ...
      desc: "Upload your resume for AI-powered ATS scoring, gap analysis and brutal honest feedback.",
      accent: "#f59e0b",
      ...
      <div className="h-cards">
        {features.map((f) =>
          f.action === "resume" ? (
            <button
              key="resume"
              className="h-card h-card--btn"
              style={{ "--accent": f.accent, "--glow": f.glow }}
              onClick={() => setShowResumeAnalyser(true)}
            >
              ...
              <div className="h-card-arrow">
                Analyse <i className="bi bi-arrow-right ms-1" />
              </div>
            </button>
          ) : (
            <Link
